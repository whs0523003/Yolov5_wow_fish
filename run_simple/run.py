...
def main():
    ...
    while True:
        ...
        if x != 0 and y != 0:
            ...
            # ------- 保存图片，可用于边钓鱼边采数据 -------
            # 将上边画框部分最后的annotator注释掉就可以得到原图了
            # tm = str(time.time())[:-8]  # 根据时间保存图片名
            # img0 = cv2.cvtColor(img0, cv2.COLOR_RGB2BGR)  # 转换图片通道数为正常颜色
            # cv2.imwrite('./run_csgo/wow_data/'+tm+'.jpg', img0)  # 保存图片
            # -----------------------------------------

            # 移动到鱼漂
            pyautogui.moveTo(x+640, y+250)
